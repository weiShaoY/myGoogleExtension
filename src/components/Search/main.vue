...
function getSearchKeyword(): string {
  ...
  // 遍历搜索引擎列表
  for (const item of SearchConfig.siteGroups) {
    if (item.siteGroups) {
      // 检查子菜单中的搜索引擎
...
<template>
  <div
    class="fixed bottom-3 left-2 z-99999 flex flex-col gap-3 overflow-hidden border rounded-xl bg-white shadow-lg transition-all duration-200 ease-in-out"
  ...
  >
    ...
    >
      <!-- 搜索引擎菜单项 -->
      <template
        v-for="item in SearchConfig.siteGroups"
        :key="item.name"
      >
        <!-- 有子菜单的项目 -->
        <el-sub-menu
          v-if="item.siteGroups"
          :index="item.name"
        >
          <template
            #title
          ...
          <el-menu-item
            v-for="subItem in item.siteGroups"
            :key="subItem.name"
            :index="subItem.name"
