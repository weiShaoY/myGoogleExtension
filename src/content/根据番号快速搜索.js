...
(function () {
  ...
  window.qxin = {} // 给引用脚本传参
  let timerGetInfo // 延时获取信息
  let timerMouseLeave // 鼠标离开番号。鼠标进入菜单时的定时器, 超时不进入, 菜单消失
  let timerMouseLeaveMenu // 鼠标离开菜单
  // var timerImgLoading;    // 图片加载后 重新定位的定时器。 加载成功后, 清除该定时器
  let avInfo = {} // 临时存储相关信息
  let avInfoID = '' // 临时存储avID
  let localInfo = {} // 从本地获取到的番号信息, 只在判断是否本地存在和存储信息时使用
  let Imgscall = 1.0 // 图片默认放大倍数, 在图片上滚动鼠标滚轮使用。
  let javdbTime = [] // 记录访问javdb的时间, 如果短时间内多次访问就限制访问, 默认是5分钟内限制为10次访问。
  let divTarget // 鼠标当前经过的番号节点
  let imgErrorSVG = 'data:image/svg+xml,%3Csvg class=\'icon\' viewBox=\'0 0 1024 1024\' xmlns=\'http://www.w3.org/2000/svg\' width=\'200\' height=\'200\'%3E%3Cpath d=\'M304.128 456.192c48.64 0 88.064-39.424 88.064-88.064s-39.424-88.064-88.064-88.064-88.064 39.424-88.064 88.064 39.424 88.064 88.064 88.064zm0-116.224c15.36 0 28.16 12.288 28.16 28.16s-12.288 28.16-28.16 28.16-28.16-12.288-28.16-28.16 12.288-28.16 28.16-28.16z\' fill=\'%23999\'/%3E%3Cpath d=\'M887.296 159.744H136.704C96.768 159.744 64 192 64 232.448v559.104c0 39.936 32.256 72.704 72.704 72.704h198.144L500.224 688.64l-36.352-222.72 162.304-130.56-61.44 143.872 92.672 214.016-105.472 171.008h335.36C927.232 864.256 960 832 960 791.552V232.448c0-39.936-32.256-72.704-72.704-72.704zm-138.752 71.68v.512H857.6c16.384 0 30.208 13.312 30.208 30.208v399.872L673.28 408.064l75.264-176.64zM304.64 792.064H165.888c-16.384 0-30.208-13.312-30.208-30.208v-9.728l138.752-164.352 104.96 124.416-74.752 79.872zm81.92-355.84l37.376 228.864-.512.512-142.848-169.984c-3.072-3.584-9.216-3.584-12.288 0L135.68 652.8V262.144c0-16.384 13.312-30.208 30.208-30.208h474.624L386.56 436.224zm501.248 325.632c0 16.896-13.312 30.208-29.696 30.208H680.96l57.344-93.184-87.552-202.24 7.168-7.68 229.888 272.896z\' fill=\'%23999\'/%3E%3C/svg%3E'
  let cid = {} // 用于预告视频的链接转换
  let localVideoList = [] // 获取本地 jellyfin/emby 中所有的视频名称

  let allHTML = document.querySelector('body') // 获取网页
  ...
  // 初始化 setting
  // setting: 相关设置选项
  let setting = GM_getValue('_setting')
  ...
  // 自定义获取信息的地址, 避免网络不通顺
  let javbusLink = setting.javbus ? setting.javbus : 'https://www.javbus.com/'
  let javDBLink = setting.javdb ? setting.javdb : 'https://javdb.com/'

  // 磁链复制功能在引用脚本中还有一部分
  window.qxin.CopyMagnet = !!setting.magnetCopy
  window.qxin.includeIDinLinks = setting.includeIDinLinks ?? false

  // 测试用
  let debug = setting.debug ? setting.debug : false
  ...
  let oRegExp = /(?<![\w/=>]|www\.|col-|\d-|Jukujo-)(?!heyzo|SHINKI|JPNXXX|carib|vps)[a-z]{2,6}-\d{2,5}(?:-c|_c|-4k)?(?!\d|[A-Z]{2,}|-\d|\.com|\.\d)|(?<![\w/\\.【\-#@=]|www\.)(?!heyzo|SHINKI|JPNXXX|carib|and|vps|dvd)[a-z]{2,6}\s{0,2}\d{3,4}(?:-c|_c)?(?![\w\-./×％%@歳分]|\s?天| 于| 发表| 發表| 歲|小时|系列| Min| day|ml| time|cm| ppi|\.com)|\b(?:PARATHD|3DSVR|STARSBD)[-\s]?\d{3,4}\b|\b(?:HIMEMIX|CASMANI|MGSSLND)[-\s]?\d{3}\b|\b(?:k|n)[01]\d{3}(?!\w|-)|(?<!\w|\d-|\/)[01]\d{5}[-_]1?\d{2,3}(?!\w|-\d)|\b(?:carib|1pondo)[-_]\d{6}[-_]\d{2,3}\b|(?<!\w|\d-)\d{6}[-_]\d{2,3}(?:-1pon|-carib|-paco)\b|(?<!\w|\d-)\d{6}_1?\d{3}_0[12](?!\w|-\d)|HEYZO[_-\s]?(?:hd_)?\d{4}/gi
  // 省略字母, 连续数字的番号 例: abc-001、002、003
  let oRegExp2 = /(?<=(?<!\w|\d-)([a-z]{2,6})(?:[\s,，、-]?(?!2022|2021|2020|2019)\d{3,4})+(?!\d)[\s,、，和跟]{0,2})\d{3,4}(?![\w％%人年歳万の発])/gi
  ...
  let oRegExp_wuma = /(?<![\w\-/])\d{3}[a-z]{2,5}[-\s]?\d{3,4}(?![\w\-]|.torrent|年)|(?<!\w|\/)FC2\D{0,5}\d{6,7}|HEYDOUGA[_-\s]?\d{4}-\d{3,5}|\bT28-\d{3}|\bT-2\d{4,5}(?!\w|-)|(?<![\w\-/])[01]\d{5}-[a-z]{2,7}(?!\w|-)|\bMKB?D-S\d{2,3}(?!\w|-)|(?:SHINKI|KITAIKE)[-\s]?\d{3}(?!\w|-)|JPNXXX[-\s]?\d{5}(?!\w|-)|xxx-av[-\s]\d{4,5}(?!\w|-)|\bcrazyasia\d{5}(?!\w|-)|\bPEWORLD\d{5}(?!\w|-)|\b[01]\d{5}[-_]?_01(?:(?=-10mu))?|Jukujo-Club-\d{3}/gi
  // 省略写的fc2番号 例: fc2-123456 567890
  let oRegExp_wuma2 = /(?<=(FC2\D{0,5})(?:[\s,、-]?\d{6,7})+[\s,、]?)\d{6,7}/gi
  ...
  let oRegExp_Exclude_ID = /^(?:fx-?([^0]\d{2}|\d{4})|[a-z]+-?0{2,6}$|pg-13|crc-32|ea211|fs[\s-]?140|trc-20|erc-20|rs[\s-]?(232|422|485)|(sg|ae|kr|tw|ph|vn|kh|ru|uk|ua|tr|th|fr|in|de|sr)[\s-]\d{2}|(gm|ga)-\d{4}|cd[\s-]?\d{2,4}|seed[\s-]?\d{3}$|pc005|moc-\d{5}|wd-40|rtd[\s-]?\d{4}|cm\d{4}|rk\d{4})|ns[\s-]?\d{3,4}/i
  // 排除在此的关键词。 个别与番号同名的也被排除, 例如 Top-10 这种
  let oRegExp_Exclude_en = new RegExp(
    '^(?:'
  ...
  let oRegExp_Special_en = /^(?:ace|akb|api|am|anime|at|be|best|bt|bl|cp|crc|exynos|dl|dp|dq|gb|girl|jd|ha|has|hc|hours|iq|in|mk|mini|mhz|mx|no|open|of|over|part|pd|pdd|porn|pt|sb|sex|tv|tb|ty|ver|vip|zd|zip)$/i
  // 在没有横杠的情况下, 会排除在此的数字
  let oRegExp_Special_num = /^(?:007|101|110|115|123|128|256|360|365|370|404|512|520|911|996|\d{1,2}00|19[789]\d|20[012]\d|720|1080|1024|2048|[05-9]\d{3}|(\d)\1{2,3})$/
  // 排除符合条件的无码番号
  let oRegExp_Exclude_wuma = /^512gb/i
  // 可能是素人、无码番号, 如果在javbus获取不到信息, 会继续从javdb中查找. 2022-10-23 v0.19.3注: 可能会被弃用
  let oRegExp_SuRen = /ANAN|ARA|BEAF|BKKJ|BSKC|BSKJ|CCDV|CUTE|DAVC|DCV|DDH|ECSN|ENE|ERKR|EROFC|FKNP|FLC|FTHT|GANA|GESB|GRQR|GRMO|GRMR|HABJ|HHL|HMDN|HMDNC|HMT|HOMEV|IMGN|IND|INSF|INSTC|JAC|JNT|JPNXXX|KING|KNB|LBJ|LOG|LUXU|MAAN|MCHT|MFC|MIUM|MKGF|MONA|NAEN|NMCH|NTK|NTR|OPCYN|OREC|ORECO|PAK|POK|pow|PPZ|PRGO|REIW|RKD|SCOH|SGK|SHE|SHINKI|SIRO|SIROR|SIMM|SQB|SROM|SSDV|SSK|STCV|STH|SUKE|TEN|TKOL|TKPR|WITH|\d{6}|(?:k|n)\d{4}/i
  // 磁力链接
  let oRegExp_Magnet = /magnet:\?xt=urn:btih:[0-9a-f]{40}|(?<![\w/|=])[0-9a-f]{40}(?!\w|-)/gi
  ...
  switch (window.location.hostname) {
    case 'www.dmm.co.jp':
      ...
      oRegExp = /[a-z]{2,6}\d{3,4}/gi
      break
    case 'attackers.net':
      console.log('当前匹配网站: attackers.net')
      oRegExp = /(?<=dvd)[a-z]{2,5}\d{3,4}/gi
      break
  ...
  // 对动态添加的dom进行检测
  let observerTarget = document.querySelector('body') // 选择目标节点
  let observerConfig = { childList: true, characterData: true, subtree: true } // 配置观察选项
  var observer = new window.MutationObserver((mutations) => { // 创建观察者对象
    // console.log("观察者数量");
  ...
  function findAndReplaceDOMTextFun(element) {
    findAndReplaceDOMText(element, {
      ...
      replace(portion, match) {
        ...
        // 添加事件和样式
        let avID = formatAVID(otext)

        // 判断3位数字写成2位的
        if (/-\d{2}$/.test(avID) && localInfo[avID] && localInfo[avID].noInfo) {
        ...
        let odiv = addEventAndStyle(localInfo[avID], avID) // 添加事件和样式
  ...
  function findAndReplaceDOMTextFun2(element) {
    findAndReplaceDOMText(element, {
      ...
      replace(portion, match) {
        let avID = `${match[1]} ${match[0]}` // 视为无横杠番号
  ...
  function findAndReplaceDOMTextFun_Wuma(element) {
    findAndReplaceDOMText(element, {
      ...
      replace(portion, match) {
        ...
        // 添加事件和样式
        let avID = formatWuma(otext) // 格式化番号
        // 123abc-456 数字字母-数字
        if (/\d{3}[a-z]{2,5}[-\s]?\d{3,4}/i.test(avID)) {
          avID = avID.slice(3)
          avID = formatAVID(avID)
        }
  ...
  function findAndReplaceDOMTextFunMagnet() {
    ...
    findAndReplaceDOMText(allHTML, {
      ...
      replace(portion, match) {
        ...
        let odiv = document.createElement('savmagnet')
  ...
  function addEventAndStyle(isExist, avID) {
    // 添加事件
    let odiv = document.createElement('savdiv')
  ...
  function displayMode(odiv, otext) {
    if (setting.addSearchButton) {
      ...
      let divWrap = document.createElement('savdiv')
  ...
  function createPattenr(id, id_wuma) {
    ...
    if (setting.list_all && setting.list_all.length > 0) {
      let savListAll = setting.list_all
    ...
    let odiv = document.createElement('avdiv')
    if (localInfo[id]) {
  ...
  function savIDClick(e) {
    ...
    let avid = e.target.dataset.av
    if (avid) {
      GM_setClipboard(avid)
      if (debug)
        console.log('粘贴成功: ', avid)
    }
    e.preventDefault()
    return false
  }
  ...
  function savMenuMouseLeave(e) {
    timerMouseLeaveMenu = setTimeout(() => {
      let odiv = document.querySelector('.sav-menu')
      if (odiv) {
        if (setting.dontClearMenu) {
          console.log('移除')
  ...
  function avmouseenter(e) {
    ...
    let wuma = e.target.dataset.av_wuma
    let avid = e.target.dataset.av
    ...
    let avdiv = document.querySelector('.sav-menu')
    if (avdiv) {
      avdiv.parentNode.removeChild(avdiv)
    }
    let oPosition = e.target.getBoundingClientRect()
    let odiv = createPattenr(avid, wuma)
    ...
    let posRight = 630 - (document.documentElement.clientWidth - divTarget.getBoundingClientRect().x)
    if (posRight < 15) {
      posRight = 15
    }
    ...
    let otherInfo = document.createElement('avdivsInfo')
  ...
  function savMenuClick(e) {
    ...
    if (!setting.dontImgBig) {
      ...
      if (e.target.tagName == 'IMG') {
        ...
        let oImg = document.querySelector('.sav-menu').querySelector('img')
        if (oImg) {
  ...
  function selectSearch(e) {
    ...
    if (selectText.length > setting.selectLength)
      return // 如果复制的文字过长,退出。避免复制网址时自己弹出。
    selectText = selectText.replace(/[^a-z0-9-]/gi, '') // 去掉一些莫名其妙的符号。网友分享的番号总是各种各样
    let avid = selectText.match(/[a-z|]{2,8}-?\d{2,6}/i)
    if (!avid)
      return // 如果没搜索到,退出
    ...
    let odiv = createPattenr(avid)

    let divClientRect = odiv.getBoundingClientRect()
    let divWidth = divClientRect.right - divClientRect.left
  ...
  function settingPostion(image) {
    ...
    if (!odiv)
      return
    let oClient = odiv.getBoundingClientRect()
    let oTop = oClient.top // 距离顶部举例
    let oHeight = oClient.height // 自身高度
    // var oWidth = oClient.Width;  // 自身宽度
    let winHeight = document.documentElement.clientHeight // 可视窗口高度
    let winWidth = document.documentElement.clientWidth // 可视窗口宽度
    ...
    if (oTop + oHeight - loadingContainerHeight > winHeight) { // 越出了屏幕底边
      ...
      let posBottom = oHeight - (document.documentElement.clientHeight - divTarget.getBoundingClientRect().y) + 30
      if (posBottom < 5) {
        posBottom = 5
      }
      ...
      onload(data) {
        ...
        if (data.status == 403 || data.status == 522) {
          ...
          if (avID.length - avID.indexOf('-') == 3) {
            ...
            setTimeout(() => {
              let avdivinfo = document.querySelector('avdivsinfo')
          ...
          else if (oRegExp_SuRen.test(avID) || (setting.getInfoFailToJavDB ?? true)) {
            getInfo_wuma_javdb1(avID)
          }
          else {
            getInfo_end(avID, data)
        ...
        // 获取演员名字
        let starNameList = htmlDoc.querySelectorAll('.star-name')
        let starName = []
        ...
        // 获取标签
        let tagList = htmlDoc.querySelectorAll('input[name=\'gr_sel\']')
        let tags = ''
        ...
        let other = htmlDoc.querySelectorAll('.header')
        ...
        // 封面
        let image = htmlDoc.querySelector('.bigImage img')
        if (image) {
          ...
          if (videoURL?.includes('dmm.co.jp')) {
            ...
            if (corp_finish.length != avIDsplit[0].length) {
              ...
              if (avInfo.date && new Date(avInfo.date) > new Date('1/1/2015')) {
                ...
                if (debug)
                  console.log('更新用户规则 cid: ', corp_finish, avID)
                GM_setValue('_setting2', setting2)
              }
            }
          }
  ...
  function getPic_dmm(avID) {
    ...
    if (cid[corp]) {
      if (debug)
        console.log('图片加载: 在默认URL规则中: ', cid[corp])
      ...
      if (debug)
        console.log('图片加载: 在用户学习URL规则中: ', cid_user[corp])
      urlPart = cid_user[corp][0].replace('00', '') + idNum
      // corp = cid_user[corp][0]
    }
    else {
      if (debug)
        console.log('图片加载: URL规则,默认')
      urlPart = corp + idNum
    }
  ...
  function getInfo_fc2_openPage(avID, link) {
    ...
    GM_xmlhttpRequest({
      ...
      onload(data) {
        ...
        let iframe = htmlDoc.querySelector('iframe.lazy')
        // 视频
  ...
  function getInfo_wuma_javdb1(avID) {
    ...
    GM_xmlhttpRequest({
      ...
      onload(data) {
        ...
        let searchResult = htmlDoc.querySelectorAll('.movie-list .item')

        let REavID = new RegExp(avID.replace(/-|_/, '[_-]'), 'i')
        ...
        if (!searchResult.length) {
          ...
          if (!htmlDoc.querySelector('title')) {
            ...
            if (htmlDoc.body.innerText.includes('copyright')) {
        ...
        let avLink = searchResult[0].querySelector('a').href
  ...
  function getInfo_wuma_javdb2(avID, link) {
    ...
    GM_xmlhttpRequest({
      ...
      onload(data) {
        ...
        // 获取相关信息, 匹配的具体的字。
        let other = htmlDoc.querySelectorAll('.panel-block')
        for (let i = 0; i < other.length; i++) {
          ...
          if (other[i].innerHTML.search('演員') > -1) {
            let starNameList = other[i].querySelectorAll('a')
            var starName = []
            for (let i = 0; i < starNameList.length; i++) {
        ...
        // 封面
        let image = htmlDoc.querySelector('.video-meta-panel img')
        image.removeAttribute('title') // 鼠标经过的时候会触发离开事件,所以删掉
  ...
  async function javdbVideoTest(link, avID) {
    await GetMess(link)
      .then((res) => {
        if (res.status == 200) {
          if (debug)
            console.log('javdb中的链接正常')
          return link
        }
        else {
          if (debug)
            console.error('javdb中的链接有误 X')
  ...
  function getInfo_end(avID, data, image) {
    ...
    if (data.status == 403) {
      ...
      if (starName && starName.length == 1 && title.includes(starName[0])) {
    ...
    localInfo[avID].getInfo_Time = Date.now()

    if (debug)
      console.log('保存相关信息: localInfo[avID] : ', localInfo[avID])
    GM_setValue('avInfo2', localInfo)
    ...
    // console.log(avInfo);
    // console.log("------------------");
    let otherInfo = document.querySelector('avdivsInfo')
    if (!otherInfo) {
  ...
  function getInfo_local(avID) {
    ...
    if (localInfo[avID].link) {
      if (localInfo[avID].link.includes(javDBLink)) {
        changeRelatedPage(localInfo[avID].link, 'JavDB 页面', avID)
      }
      else if (localInfo[avID].link.includes(javbusLink)) {
        changeRelatedPage(localInfo[avID].link, 'JavBus 页面', avID)
      }
      else if (localInfo[avID].link.includes('fc2hub')) {
  ...
  function reloadGetInfo(avID) {
    ...
    if (!localInfo[avID].getInfo_Time || nowTime - localInfo[avID].getInfo_Time > 43200000) {
      if (oRegExp.test(avID)) {
        getInfo(avID)
      }
      else if (/fc2/i.test(avID)) {
        getInfo_fc2(avID)
      }
      else {
        getInfo_wuma_javdb1(avID)
  ...
  function addOtherInfo(avID) {
    ...
    if (avInfo.starName && avInfo.starName.length > 0) {
      ...
      for (let i = 0; i < avInfo.starName.length; i++) {
  ...
  function addLoading(odiv) {
    let savLoading = document.createElement('avdiv')
    savLoading.classList.add('savLoadingContainer')
    let savhr = '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" version="1.1">'
  ...
  function removeLoading() {
    // return;
    let loading = document.querySelector('.savLoadingContainer')
    if (loading) {
  ...
  function formatAVID(otext) {
    otext = otext.replace(/\s+|-c|_c|-4k|carib[-_]|1pondo[-_]|-1pon|-paco|-carib|hd_/gi, '')
    if (/^[a-z|]{2,8}\d{2,5}$/i.test(otext)) {
      let oindex = otext.search(/\d/)
      if (oindex > 0) {
  ...
  function formatWuma(otext) {
    otext = otext.replace(/\s+|carib[-_]|1pondo[-_]|-1pon|-paco|-carib|hd_/gi, '')
    ...
    if (/t28|t-|MKD-S|SHINKI|KITAIKE|JPNXXX|xxx-av|crazyasia|PEWORLD|MKBD-S/i.test(otext)) {
      return otext.toUpperCase()
    }
    // heydouga
    if (/HEYDOUGA/i.test(otext)) {
      return `heydouga-${otext.slice(otext.search(/\d/))}`
    }
  ...
  function IDcheck(otext) {
    let oOnlyText = otext.replace(/[^a-z]/gi, '') // 番号中的英文
    let oOnlyNum = otext.replace(/\D/g, '') // 番号中的数字
  ...
  function IDcheckWuma(otext) {
    if (/\d{3}[a-z]{2,5}[-\s]?\d{3,4}/i.test(otext)) {
      if (/^cm$/i.test(otext.replace(/[^a-z]/gi, ''))) {
        return true
      }
    }
    if (oRegExp_Exclude_wuma.test(otext)) {
  ...
  function checkParentClass(startDom) {
    if (startDom.classList && startDom.classList.length && startDom.className.match(window.qxin.RE_Exclude_className)) {
      if (debug)
        console.log('checkParentClass: 父元素存在相关class, 已排除: ', startDom.className)
      return true
    }
    if (startDom.parentElement && startDom.parentElement.nodeName !== 'body') {
  ...
  function fuzzyMatch(avID, title) {
    ...
    let onlyText = avID.replace(/[^a-z]/gi, '') // 番号中的英文
    let onlyNum = avID.replace(/\D/g, '') // 番号中的数字

    // 数字都是错误的情况下, 直接返回
    if (!title.includes(onlyNum)) {
      console.log('数字错误')
      return false
    }
  ...
  function googleTrans(avID, transText) {
    ...
    let translate_url = ''
    let googleTransApi = 'https://translate.google.com.hk/translate_a/single?client=gtx&dt=t&dj=1&sl=auto&tl=zh-CN&hl=zh-CN&q='
    ...
    GM_xmlhttpRequest({
      ...
      onload(r) {
        setTimeout(() => {
          ...
          if (!r.responseText.includes('sentences')) {
          ...
          for (let i = 0; i < data.sentences.length; i++) {
            let getransCont = data.sentences[i]
  ...
  function baiduTrans(avID, transText) {
    ...
    let appid = setting.baiduAppid
    let key = setting.baiduKey
    let salt = Date.now()
    // var query = 'apple';
    // 多个query可以用\n连接  如 query='apple\norange\nbanana\npear'
    // var from = 'jp';
    // var to = 'zh';
    let str1 = appid + transText + salt + key
    let sign = MD5(str1)
    let translate_url = `http://api.fanyi.baidu.com/api/trans/vip/translate?q=${encodeURIComponent(transText)}&from=jp&to=zh&appid=${appid}&salt=${salt}&sign=${sign}`
    GM_xmlhttpRequest({
      method: 'GET',
      url: translate_url,
      onload(r) {
  ...
  async function queryDMMVideoURL(avID, host = 'cc3001.dmm.com') {
    ...
    if (cid[corp]) {
      if (debug)
        console.log('视频加载: 在默认URL规则中: ', cid[corp])
      if (cid[corp].length > 1) {
        console.log('多个')
        // 此处只会处理第2个及之后的, 第一个依旧会判断是否含0, 请求两遍
      ...
      if (debug)
        console.log('视频加载: 在用户学习URL规则中: ', cid_user[corp])
      urlPart = cid_user[corp][0] + idNum
      // corp = cid_user[corp][0]
    }
    else {
      if (debug)
        console.log('视频加载: URL规则,默认添加00')
  ...
  function getJellyfin(avID) {
    ...
    GM_xmlhttpRequest({
      ...
      timeout: 1000,
      onload(r) {
        let div_jellyfin = document.querySelector('.jellyfin')
    ...
    GM_xmlhttpRequest({
      ...
      onload(r) {
        ...
        if (data.TotalRecordCount) {
          let div_actor = document.querySelector(`.sav-actors-${index}`)
          if (div_actor) {
            let div_jellyfin = document.createElement('avspan')
            if (setting.emby) {
            ...
            // 插入到演员dom的后面
            let div_parent = div_actor.parentNode
  ...
  function actor_search() {
    ...
    // 'X-Emby-Token': apiKey,
    // };
    let url = `${setting.jellyfinHost}api/Persons&api_key=${setting.jellyfinApiKey}`
  ...
  function qBit(torrent) {
    ...
    let qbi_host = setting.qBitHost
    let download = setting.qBitDownload
    GM_xmlhttpRequest({
      method: 'POST',
      ...
      onload(response) {
        ...
        if (response.statusText) {
          if (setting.qBitNoPopup && response.statusText.includes('OK')) {
            return
          }
  ...
  function savBoxClose() {
    let box = document.querySelector('#sav-editCodeBox')
    if (box) {
  ...
  function savBoxGetValue() {
    let codevalue = document.querySelector('#sav-editCodeBox textarea').value
