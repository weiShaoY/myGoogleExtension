...
(function () {
  ...
  function extractCodeFromTitleElement(containerEl) {
    ...
    const isPureLetters = /^[a-z]+$/i.test(cleanedStrong.replace(/\s+/g, ''))
  ...
  function normalizeCode(code) {
    ...
    const isPureLetters = /^[a-z]+$/i.test(normalized)
  ...
  function init() {
    ...
    // 确定当前页面类型
    CONFIG.currentPageType = window.location.href.includes('watched_videos')
      ? 'watched'
    ...
    if (isImporting && CONFIG.currentPageType) {
      ...
      if (importType === CONFIG.currentPageType) {
        ...
        setTimeout(() => {
          ...
          CONFIG.importedCount = importedCount ? Number.parseInt(importedCount) : 0
  ...
  function applyBlockEffect() {
    ...
    movieItems.forEach((item) => {
      // 清除所有相关类
      item.classList.remove('javdb-blocked', 'javdb-watched', 'javdb-wanted', 'javdb-low-score', 'javdb-normal-score', 'javdb-high-score', 'javdb-excellent', 'javdb-search-match')
  ...
  function applyScoreHighlight(item) {
    ...
    if (scoreMatch) {
      ...
      reviewCount = Number.parseInt(scoreMatch[2])
    }
    else {
      // 宽松匹配：只提取分数
      ...
      score = Number.parseFloat(looseMatch[1])
  ...
  function bindMagicLensEvents() {
    ...
    document.addEventListener('mousemove', (e) => {
      // 检查开关状态
      if (!CONFIG.enableImagePreview || !LensState.isVisible)
        return
  ...
  function goToNextPage() {
    ...
    if (hasMoreContent) {
      ...
      if (pageMatch) {
        const currentPage = Number.parseInt(pageMatch[1])
  ...
  function validateImportData(data) {
    if (!data || typeof data !== 'object')
      return false
    if (!Array.isArray(data.watched) && !Array.isArray(data.wanted))
      return false
    // 至少需要一个列表
    return true
  }
  ...
  function bindVideoDetailButtons(retryCount = 0) {
    ...
    if (!videoCode) {
      ...
      if (retryCount < 5) {
        setTimeout(bindVideoDetailButtons, 500, retryCount + 1)
      }
      return
    }
